...
addRoute('GET',  '/api-services/{reference}/status',              function ($p)  { (new ApiRequestController())->checkStatus($p['reference']); });
addRoute('POST', '/api-services/{reference}/status',              function ($p)  { (new ApiRequestController())->checkStatus($p['reference']); });
